...
import AdminStandingsManager from './AdminStandingsManager'
import ManagerApplications from './ManagerApplications'
import ManagerLog from './ManagerLog'
import AdminReleaseModal from './AdminReleaseModal'
...
function AdminPanel() {
  ...
  const [releasePlayer, setReleasePlayer] = useState(null)
  ...
  useEffect(() => {
    ...
    const sub = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'club_budgets' }, fetchData)
      .subscribe()
  ...
  if (!isAdmin) {
    return (
      <div className="crf-card" style={{ textAlign: 'center', padding: '60px 24px' }}>
        ...
        <p style={{ color: 'var(--crf-text-muted)' }}>You must be an admin to access this panel.</p>
      </div>
    )
  }
  ...
  const handleAddClub = async (name) => {
    ...
    if (error) showMessage('Failed to create club', 'error')
      ...
      // Auto-init budget
      await supabase.from('club_budgets').upsert(
        { club_name: name, budget: 100000000, spent: 0, earned: 0 },
        { onConflict: 'club_name' }
      )
      showMessage('Club created!')
  ...
  const handleDeleteClub = async (id) => {
    ...
    if (error) showMessage('Failed to delete club', 'error')
      ...
      if (club) {
        await supabase.from('club_budgets').delete().eq('club_name', club.name)
      }
      showMessage('Club deleted!')
    }
  }
  ...
  const handleAdminRelease = async (player, reason, compensation) => {
    try {
      ...
      // Update club budget if compensation > 0
      if (compensation > 0 && player.club) {
        const clubBudget = budgets.find(b => b.club_name === player.club)
        if (clubBudget) {
          await supabase
            .from('club_budgets')
            .update({ earned: (clubBudget.earned || 0) + compensation })
            .eq('id', clubBudget.id)
        }
      }
      ...
      await supabase.from('manager_logs').insert([{
        ...
        amount: compensation,
        details: { type: 'admin_release', reason, compensation },
      }])

      showMessage(`${player.roblox_username} released. Compensation: $${compensation.toLocaleString()}`, 'success')
  ...
  const handleBudgetEdit = async (budgetId, newBudget) => {
    try {
      ...
      await supabase.from('manager_logs').insert([{
        manager_username: 'Admin',
        club: budget?.club_name,
        action: 'budget_adjusted',
        amount: parseInt(newBudget),
        details: { type: 'admin_budget_edit', previous_budget: budget?.budget },
      }])

      showMessage('Budget updated!')
      setShowBudgetEdit(null)
    } catch (err) {
      showMessage('Failed to update budget: ' + err.message, 'error')
    }
  }
  ...
  const tabs = [
    ...
    { key: 'standings', label: 'Standings', icon: BarChart3, count: null },
    { key: 'competitions', label: 'Competitions', icon: Swords, count: null },
    { key: 'applications', label: 'Applications', icon: Briefcase, count: null },
    { key: 'manager_log', label: 'Manager Log', icon: ScrollText, count: null },
  ]
  ...
  const getClubBudget = (clubName) => budgets.find(b => b.club_name === clubName)
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ color: 'var(--crf-text-muted)', fontSize: '0.875rem' }}>
          Manage players, clubs, transfers, standings, budgets, and manager applications
        </p>
      </div>
        ...
        {[
          ...
          <div key={i} className="crf-card" style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '0.8rem', color: 'var(--crf-text-muted)' }}>{stat.label}</div>
            </div>
          </div>
        ))}
      </div>
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
            <div style={{ position: 'relative', flex: 1, minWidth: '250px', maxWidth: '400px' }}>
              <Search size={18} style={{ position: 'absolute', left: '14px', top: '50%', transform: 'translateY(-50%)', color: 'var(--crf-text-muted)' }} />
              <input
                type="text"
              ...
              <table className="crf-table">
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr><td colSpan="10" style={{ textAlign: 'center', padding: '40px', color: 'var(--crf-text-muted)' }}>No players found</td></tr>
                  ) : (
                      ...
                      <tr key={p.id}>
                        ...
                        <td>
                          <div style={{ display: 'flex', gap: '4px' }}>
                            ...
                            <button onClick={() => { setReleasePlayer(p); setShowReleaseModal(true); }} className="crf-btn crf-btn-sm" style={{ padding: '4px', background: 'rgba(245, 158, 11, 0.2)', color: '#f59e0b' }} title="Admin Release"><DollarSign size={12} /></button>
                          </div>
                        </td>
                      </tr>
                    ))
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '16px' }}>
            {clubs.map(club => {
              ...
              const budget = getClubBudget(club.name)
              return (
                <div key={club.id} className="crf-card">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      ...
                      <div>
                        ...
                        <p style={{ fontSize: '0.8rem', color: 'var(--crf-text-muted)' }}>{clubPlayers.length}/21 players</p>
                      </div>
                    </div>
                  ...
                  {budget ? (
                    <div style={{ marginBottom: '12px' }}>
                      <BudgetDisplay budget={budget.budget} spent={budget.spent} earned={budget.earned} compact />
                    </div>
                  ) : (
                    <div style={{ marginBottom: '12px', fontSize: '0.8rem', color: 'var(--crf-text-muted)' }}>No budget data</div>
                  )}

                  <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
                    <button
                      onClick={() => setShowBudgetEdit(budget)}
                      className="crf-btn crf-btn-sm crf-btn-secondary"
                    >
                      <Wallet size={12} /> Edit Budget
                    </button>
                  </div>
      ...
      {activeTab === 'standings' && <AdminStandingsManager />}
      {activeTab === 'competitions' && <AdminMatchesManager />}
      {activeTab === 'applications' && <ManagerApplications />}
      {activeTab === 'manager_log' && <ManagerLog />}

      {/* Budget Edit Modal */}
      {showBudgetEdit && (
        <div className="modal-overlay" onClick={() => setShowBudgetEdit(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '400px' }}>
            <div style={{ padding: '24px', borderBottom: '1px solid var(--crf-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ fontSize: '1.3rem', fontWeight: 700, fontFamily: 'Oswald, sans-serif' }}>Edit Budget: {showBudgetEdit.club_name}</h2>
              <button onClick={() => setShowBudgetEdit(null)} style={{ background: 'none', border: 'none', color: 'var(--crf-text-muted)', cursor: 'pointer' }}><X size={24} /></button>
            </div>
            <form onSubmit={e => { e.preventDefault(); handleBudgetEdit(showBudgetEdit.id, e.target.budget.value) }} style={{ padding: '24px' }}>
              <div style={{ marginBottom: '16px' }}>
                <label style={{ display: 'block', marginBottom: '6px', fontSize: '0.875rem', fontWeight: 600, color: 'var(--crf-text-muted)' }}>Total Budget ($)</label>
                <input type="number" name="budget" defaultValue={showBudgetEdit.budget} className="crf-input" min="0" required />
              </div>
              <div style={{ display: 'flex', gap: '12px' }}>
                <button type="submit" className="crf-btn crf-btn-primary"><Save size={16} /> Save</button>
                <button type="button" onClick={() => setShowBudgetEdit(null)} className="crf-btn crf-btn-secondary">Cancel</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showReleaseModal && releasePlayer && (
        <AdminReleaseModal
          player={releasePlayer}
          onClose={() => { setShowReleaseModal(false); setReleasePlayer(null); }}
        ...
        <div className="modal-overlay" onClick={() => setShowAddClub(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <div style={{ padding: '24px', borderBottom: '1px solid var(--crf-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button onClick={() => setShowAddClub(false)} style={{ background: 'none', border: 'none', color: 'var(--crf-text-muted)', cursor: 'pointer' }}><X size={24} /></button>
            </div>
            <form onSubmit={e => { e.preventDefault(); handleAddClub(e.target.clubName.value.trim()) }} style={{ padding: '24px' }}>
              <label style={{ display: 'block', marginBottom: '6px', fontSize: '0.875rem', fontWeight: 600, color: 'var(--crf-text-muted)' }}>Club Name</label>
        ...
        <div className="modal-overlay" onClick={() => setShowAddTransfer(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <div style={{ padding: '24px', borderBottom: '1px solid var(--crf-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button onClick={() => setShowAddTransfer(false)} style={{ background: 'none', border: 'none', color: 'var(--crf-text-muted)', cursor: 'pointer' }}><X size={24} /></button>
            </div>
            <form onSubmit={e => { e.preventDefault(); handleAddTransfer({ player_id: e.target.player_id.value, asking_price: e.target.asking_price.value, description: e.target.description.value }) }} style={{ padding: '24px' }}>
              <div style={{ display: 'grid', gap: '16px' }}>
                <div>
                  <label style={{ display: 'block', marginBottom: '6px', fontSize: '0.875rem', fontWeight: 600, color: 'var(--crf-text-muted)' }}>Select Player *</label>
                ...
                <div>
                  <label style={{ display: 'block', marginBottom: '6px', fontSize: '0.875rem', fontWeight: 600, color: 'var(--crf-text-muted)' }}>Asking Price ($)</label>
                  <input type="number" name="asking_price" className="crf-input" placeholder="e.g. 5000000" min="0" required />
                </div>
                <div>
                  <label style={{ display: 'block', marginBottom: '6px', fontSize: '0.875rem', fontWeight: 600, color: 'var(--crf-text-muted)' }}>Description (Optional)</label>
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={e => e.stopPropagation()}>
        <div style={{ padding: '24px', borderBottom: '1px solid var(--crf-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: 'var(--crf-text-muted)', cursor: 'pointer' }}><X size={24} /></button>
        </div>
        <form onSubmit={handleSubmit} style={{ padding: '24px' }}>
          <div style={{ display: 'grid', gap: '16px' }}>
            <div>
              <label style={{ display: 'block', marginBottom: '6px', fontSize: '0.875rem', fontWeight: 600, color: 'var(--crf-text-muted)' }}>Roblox Username *</label>
              ...
              {!player && <p style={{ fontSize: '0.75rem', color: 'var(--crf-text-muted)', marginTop: '4px' }}>Avatar and User ID will be fetched automatically.</p>}
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              <div>
                <label style={{ display: 'block', marginBottom: '6px', fontSize: '0.875rem', fontWeight: 600, color: 'var(--crf-text-muted)' }}>Club</label>
              ...
              <div>
                <label style={{ display: 'block', marginBottom: '6px', fontSize: '0.875rem', fontWeight: 600, color: 'var(--crf-text-muted)' }}>Position *</label>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
              <div><label style={{ display: 'block', marginBottom: '6px', fontSize: '0.875rem', fontWeight: 600, color: 'var(--crf-text-muted)' }}>Goals</label><input type="number" name="goals" value={formData.goals} onChange={handleChange} className="crf-input" min="0" /></div>
              <div><label style={{ display: 'block', marginBottom: '6px', fontSize: '0.875rem', fontWeight: 600, color: 'var(--crf-text-muted)' }}>Assists</label><input type="number" name="assists" value={formData.assists} onChange={handleChange} className="crf-input" min="0" /></div>
              <div><label style={{ display: 'block', marginBottom: '6px', fontSize: '0.875rem', fontWeight: 600, color: 'var(--crf-text-muted)' }}>{isGoalkeeper ? 'Saves' : 'Tackles'}</label><input type="number" name={isGoalkeeper ? 'saves' : 'tackles'} value={isGoalkeeper ? formData.saves : formData.tackles} onChange={handleChange} className="crf-input" min="0" /></div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              <div><label style={{ display: 'block', marginBottom: '6px', fontSize: '0.875rem', fontWeight: 600, color: 'var(--crf-text-muted)' }}>Yellow Cards</label><input type="number" name="yellow_cards" value={formData.yellow_cards} onChange={handleChange} className="crf-input" min="0" /></div>
              <div><label style={{ display: 'block', marginBottom: '6px', fontSize: '0.875rem', fontWeight: 600, color: 'var(--crf-text-muted)' }}>Red Cards</label><input type="number" name="red_cards" value={formData.red_cards} onChange={handleChange} className="crf-input" min="0" /></div>
            </div>
          </div>
          <div style={{ display: 'flex', gap: '12px', marginTop: '24px' }}>
