...
const AdminStandingsManager = () => {
  ...
  const deleteTeam = async (id) => {
    ...
    try {
      ...
      if (error) throw error
      setTeams(prev => prev.filter(t => t.id !== id))
      showMessage('Team removed', 'success')
    } catch (err) {
  ...
  const saveTeams = async () => {
    ...
    try {
      ...
      // Update existing teams one by one
      ...
      showMessage('All standings saved!', 'success')
      await fetchTeams()
    } catch (err) {
  ...
  const getPreviewStats = (team) => {
    const played = (parseInt(team.wins) || 0) + (parseInt(team.draws) || 0) + (parseInt(team.losses) || 0)
    const points = (parseInt(team.wins) || 0) * 3 + (parseInt(team.draws) || 0)
    const gd = (parseInt(team.goals_for) || 0) - (parseInt(team.goals_against) || 0)
  ...
  const sortedPreview = [...teams].sort((a, b) => {
    ...
    return (parseInt(b.goals_for) || 0) - (parseInt(a.goals_for) || 0)
  })
    ...
    <div className="animate-fade-in">
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ color: 'var(--crf-text-muted)', fontSize: '0.85rem', margin: 0 }}>
          Add and edit league table entries. Points and GD auto-calculate.
        </p>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          <button onClick={addTeam} className="crf-btn crf-btn-primary" style={{ padding: '8px 16px', fontSize: '0.8rem' }}>
            <Plus size={16} /> Add Team
          </button>
          <button onClick={saveTeams} disabled={saving} className="crf-btn" style={{
      ...
      {teams.length > 0 && (
        <div style={{
          background: 'var(--crf-card)', border: '1px solid var(--crf-border)',
            ...
            <table className="crf-table">
              ...
              <tbody>
                {teams.map((team, index) => {
                  ...
                  return (
                    <tr key={team.id}>
                      <td style={{ fontWeight: 600, color: 'var(--crf-text-muted)' }}>{index + 1}</td>
                      <td>
                        <input
                          type="text"
                          value={team.name || ''}
                      ...
                      <td style={{ textAlign: 'center' }}>
                        <button onClick={() => deleteTeam(team.id)} className="crf-btn crf-btn-sm crf-btn-danger" style={{ padding: '6px' }}>
                          <Trash2 size={14} />
                        </button>
                      </td>
                    </tr>
